...
def ambilgambar():
    cond, img =cap.read()
    out = cv2.imwrite("filename.jpg",img)
...
def maskersuhu():
    cv2.putText(img, maskerada , orgbaris1, font, font_scale, wfontnoface, thickness, cv2.LINE_AA)
    cv2.putText(img, suhubadan + str(nilaisuhu), orgbaris3, font, font_scale, wfontwmask, thickness, cv2.LINE_AA)
    cv2.putText(img, pesanakhir2 , orgbaris4, font, font_scale, wfontnoface, thickness, cv2.LINE_AA)
